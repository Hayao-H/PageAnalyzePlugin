...
import { ContentSrcIdImpl, ContentSrcIdSetImpl, RequestImpl } from "../../../lib/net/types/impl/session/requestImpl";
import { ContentSrcIdSet, Request } from "../../../lib/net/types/json/session/request";
...
export class DataComposerImpl implements DataComposer {
    ...
    public Create(sessionInfo: SessionInfo): Request {
        ...
        request.session.keep_method.heartbeat.lifetime = sessionInfo.HeartbeatLifetime;
        request.session.protocol.name = "http";
        request.session.protocol.parameters.http_parameters.parameters.hls_parameters.use_ssl = "yes";
        request.session.protocol.parameters.http_parameters.parameters.hls_parameters.use_well_known_port = "yes";
        request.session.protocol.parameters.http_parameters.parameters.hls_parameters.transfer_preset = sessionInfo.TransferPriset;
        request.session.protocol.parameters.http_parameters.parameters.hls_parameters.segment_duration = 6000;
        request.session.session_operation_auth.session_operation_auth_by_signature.signature = sessionInfo.Signature;
        request.session.session_operation_auth.session_operation_auth_by_signature.token = sessionInfo.Token;
        request.session.content_auth.auth_type = sessionInfo.AuthType;
        request.session.content_auth.service_id = "nicovideo";
        request.session.content_auth.content_key_timeout = sessionInfo.ContentKeyTimeout;
        ...
        request.session.content_src_id_sets.push(this.GetContentSrcIDSets(sessionInfo));
